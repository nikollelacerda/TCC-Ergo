import { 
    POMODORO_DURACAO_PADRAO,
    POMODORO_INTERVALO_PAUSA, 
    POMODORO_PAUSA_LONGA, 
    POMODORO_TITULO_PADRAO 
} from "./constants";
...
export default class Timer {
    ...
    _tick(increment: number = 1000): void {
        this.time += increment;
    ...
    getTime(): {
        ...
        let val = [];
        for (let i = 0; i < 3; i++) {
            val.push(Math.floor(this.time / ((60 ** i) * 1000)) % 60);
        }
        return {
            "raw": this.time,
            "hours": val[2],
